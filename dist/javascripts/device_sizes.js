/*
    taken in part from these two awesome sources:
    http://www.mydevice.io/devices/
    http://www.canbike.org/CSSpixels/
    found some wrong values here and there,
    but compared each other and tried to normalize and fix
*/

module.exports = {
    phones: [
        {
            name: 'Apple iPhone 6+',
            width: '414',
            height: '736'
        },
        {
            name: 'Apple iPhone 6',
            width: '375',
            height: '667'
        },
        {
            name: 'Apple iPhone 5',
            width: '320',
            height: '568'
        },
        {
            name: 'Apple iPhone 4, 3',
            width: '320',
            height: '480'
        },
        {
            name: 'Apple iPod Touch',
            width: '320',
            height: '568'
        },
        {
            name: 'Samsung Galaxy Note 3, 2',
            width: '360',
            height: '640'
        },
        {
            name: 'Samsung Galaxy Note',
            width: '400',
            height: '640'
        },
        {
            name: 'Samsung Galaxy S5, S4, S3',
            width: '360',
            height: '640'
        },
        {
            name: 'Samsung Galaxy S4 mini',
            width: '360',
            height: '640'
        },
        {
            name: 'Samsung Galaxy S3 mini, S2, S',
            width: '320',
            height: '533'
        },
        {
            name: 'Samsung Galaxy Nexus',
            width: '360',
            height: '600'
        },
        {
            name: 'HTC One, X, X+',
            width: '360',
            height: '640'
        },
        {
            name: 'HTC Desire, Desire X',
            width: '480',
            height: '800'
        },
        {
            name: 'HTC One M9',
            width: '360',
            height: '640'
        },
        {
            name: 'HTC 8X',
            width: '320',
            height: '480'
        },
        {
            name: 'HTC Evo 3D',
            width: '360',
            height: '640'
        },
        {
            name: 'Motorola Nexus 6',
            width: '412',
            height: '690'
        },
        {
            name: 'LG G3',
            width: '360',
            height: '640'
        },
        {
            name: 'LG Optimus G',
            width: '384',
            height: '640'
        },
        {
            name: 'LG Optimus',
            width: '424',
            height: '753'
        },
        {
            name: 'LG Nexus 5',
            width: '360',
            height: '640'
        },
        {
            name: 'LG Nexus 4',
            width: '384',
            height: '640'
        },
        {
            name: 'Nokia Lumia (all)',
            width: '320',
            height: '533'
        },
        {
            name: 'Sony Xperia Z, S, P',
            width: '360',
            height: '640'
        },
        {
            name: 'Sony Xperia Z3',
            width: '360',
            height: '598'
        },
        {
            name: 'Blackberry Leap',
            width: '390',
            height: '695'
        },
        {
            name: 'Blackberry Passport',
            width: '504',
            height: '504'
        },
        {
            name: 'Blackberry Classic',
            width: '390',
            height: '390'
        },
        {
            name: 'Blackberry Q10',
            width: '346',
            height: '346'
        },
        {
            name: 'Blackberry Z30',
            width: '360',
            height: '640'
        },
        {
            name: 'Blackberry Z10',
            width: '384',
            height: '640'
        },
        {
            name: 'Blackberry Torch 9800',
            width: '360',
            height: '480'
        },
        {
            name: 'ZTE Open (Firefox OS)',
            width: '320',
            height: '480'
        }
    ],
    tablets: [
        {
            name: 'Apple iPad (all)',
            width: '768',
            height: '1024'
        },
        {
            name: 'Microsoft Surface',
            width: '768',
            height: '1366'
        },
        {
            name: 'Microsoft Surface Pro',
            width: '720',
            height: '1280'
        },
        {
            name: 'Kindle Fire HD 7"',
            width: '533',
            height: '853'
        },
        {
            name: 'Kindle Fire HD 8.9"',
            width: '800',
            height: '1280'
        },
        {
            name: 'Kindle Fire',
            width: '600',
            height: '1024'
        },
        {
            name: 'Samsung Galaxy Tab 3, 2, (10")',
            width: '800',
            height: '1280'
        },
        {
            name: 'Samsung Galaxy Tab (8.9")',
            width: '800',
            height: '1280'
        },
        {
            name: 'Samsung Galaxy Tab 2 (7")',
            width: '600',
            height: '1024'
        },
        {
            name: 'Samsung Nexus 10',
            width: '800',
            height: '1280'
        },
        {
            name: 'Asus Nexus 7 (v2)',
            width: '600',
            height: '960'
        },
        {
            name: 'Blackberry Playbook',
            width: '600',
            height: '1024'
        },
        {
            name: 'LG G Pad 8.3',
            width: '600',
            height: '960'
        },
        {
            name: 'Asus Nexus 7 (v1)',
            width: '600',
            height: '960'
        },
        {
            name: 'Asus Nexus 7 (v2)',
            width: '604',
            height: '966'
        },
        {
            name: 'HTC Nexus 9',
            width: '768',
            height: '1024'
        },
        {
            name: 'Google Nexus 7',
            width: '604',
            height: '966'
        }
    ]
};
